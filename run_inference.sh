...
mount -o remount,size=256G /dev/shm || true
...
CKPT_DIR=${1:-"ckpts/Z-Image-Turbo"}
SAVE_PATH=${2:-"example.png"}
PROMPT=${3:-"Young Chinese woman in red Hanfu, intricate embroidery. Impeccable makeup, red floral forehead pattern. Elaborate high bun, golden phoenix headdress, red flowers, beads. Holds round folding fan with lady, trees, bird. Neon lightning-bolt lamp (⚡️), bright yellow glow, above extended left palm. Soft-lit outdoor night background, silhouetted tiered pagoda (西安大雁塔), blurred colorful distant lights."}

python3 inference.py --ckpt_dir "${CKPT_DIR}" --prompt "${PROMPT}" --save_path "${SAVE_PATH}"
