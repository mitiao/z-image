...
rope_dict = {}
wa_rms_norm_set = {torch.Size([1, 4224, 3840]), torch.Size([1, 4096, 3840]), torch.Size([128, 2560]), torch.Size([1, 128, 30, 128]), torch.Size([1, 128, 3840])}
...
def fused_rotary_emb(query: torch.Tensor, key: torch.Tensor, freqs_cis: torch.Tensor, batch_size: int) -> torch.Tensor:
    with torch.supa.amp.autocast('supa', dtype=torch.float32):
        _, seq_len, head_dim = query.shape 

        if seq_len not in rope_dict:
            freqs = torch.view_as_real(freqs_cis.unsqueeze(2))
            cos_freqs = freqs[..., 0]
            sin_freqs = freqs[..., 1]
            expand_factor = sin_freqs.shape[-1]
            sin_freqs = sin_freqs.view(-1, expand_factor).unsqueeze(-1).expand(seq_len * batch_size, expand_factor, 2).reshape(seq_len * batch_size, expand_factor * 2)
            cos_freqs = cos_freqs.view(-1, expand_factor).unsqueeze(-1).expand(seq_len * batch_size, expand_factor, 2).reshape(seq_len * batch_size, expand_factor * 2)
            rope_dict[seq_len] = [torch.arange(0, seq_len, 1, dtype=torch.int32, device=query.device), sin_freqs, cos_freqs]

        positions, sin_freqs, cos_freqs = rope_dict[seq_len]
        rope_type = "Wan"

        query_supa, key_supa = torch_br.supa_rope_infer_v2(
            query,
            key,
            sin_freqs,
            cos_freqs,
        ...
        return query_supa.type_as(query), key_supa.type_as(key)
...
class ZImageAttention(nn.Module):
    ...
    def forward(
        ...
        query = torch_br.supa_shape_transform_qkv(
            query, bs, seq_len, self.n_heads, self.head_dim, True
        )

        key = torch_br.supa_shape_transform_qkv(
            key, bs, seq_len, self.n_kv_heads, self.head_dim, True
        )

        value = torch_br.supa_shape_transform_qkv(
            value, bs, seq_len, self.n_kv_heads, self.head_dim, True
        )
        ...
        if freqs_cis is not None:
            query, key = fused_rotary_emb(query, key, freqs_cis, bs)
...
class RopeEmbedder:
    ...
    def __call__(self, ids: torch.Tensor):
        ...
        for i in range(len(self.axes_dims)):
            index = ids[:, i]
            real_tensor = self.freqs_cis[i].real[index]
            imag_tensor = self.freqs_cis[i].imag[index]
            result_real.append(real_tensor)
            result_imag.append(imag_tensor)
        return torch.complex(torch.cat(result_real, dim=-1), torch.cat(result_imag, dim=-1))
...
class ZImageTransformer2DModel(nn.Module):
    ...
    def unpatchify(self, x: List[torch.Tensor], size: List[Tuple], patch_size, f_patch_size) -> List[torch.Tensor]:
        ...
        for i in range(bsz):
            ...
            x[i] = (
                ...
                .reshape(self.out_channels, F, H // pH, pH, W // pW, pW) # split reshape to avoid unsupporting high dims reshape
                .reshape(self.out_channels, F, H, W // pW, pW)
    ...
    def patchify_and_embed(
        ...
        for _, (image, cap_feat) in enumerate(zip(all_image, all_cap_feats)):
            ...
            image = image.cpu().permute(1, 3, 5, 2, 4, 6, 0).reshape(F_tokens * H_tokens * W_tokens, pF * pH * pW * C).to(device=device)
    ...
    def forward(
        ...
        for i in range(bsz):
            ...
            x_real_i = x_freqs_cis[i].real[:x_len]
            x_imag_i = x_freqs_cis[i].imag[:x_len]

            cap_real_i = cap_freqs_cis[i].real[:cap_len]
            cap_imag_i = cap_freqs_cis[i].imag[:cap_len]

            x_cap_real = torch.cat([x_real_i, cap_real_i])
            x_cap_imag = torch.cat([x_imag_i, cap_imag_i])

            x_cap = torch.complex(x_cap_real, x_cap_imag)
            unified_freqs_cis.append(x_cap)
