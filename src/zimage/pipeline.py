...
def generate(
    ...
    logger_if = lambda msg, cond: logger.info(msg) if cond else None
    logger_if(f"Generating image: {height}x{width}, steps={num_inference_steps}, cfg={guidance_scale}", print_time)
    ...
    t1 = time.time()
    logger_if(f"Tokenizer time taken: {t1 - t0:.2f} seconds", print_time)
    ...
    logger_if(f"Sampling loop start: {num_inference_steps} steps", print_time)

    from tqdm import tqdm
    if is_profile:
        local_rank = 0
        worker_name = f"rank{local_rank}_dp{local_rank}"
        profiler = torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,
                                                torch.profiler.ProfilerActivity.SUPA],
                                    schedule=torch.profiler.schedule(skip_first=1, wait=1, warmup=0, active=2, repeat=1),
                                    on_trace_ready=torch.profiler.tensorboard_trace_handler('hf-training-trainer', worker_name=worker_name),
                                    profile_memory=True,
                                    with_stack=True,
                                    record_shapes=True,
                                    use_supa_simple=False,)
        profiler.start()

    # Denoising loop with progress bar
    for i, t in enumerate(tqdm(timesteps, desc="Denoising", total=len(timesteps), disable=not print_time)):
    ...
    # transformer.to("cpu")
    # text_encoder.to("cpu")
    # torch_br.supa.empty_cache()
    # gc.collect()
    if output_type == "latent":
        return latents

    t2 = time.time()
    logger_if(f"Dit time taken: {t2 - t1:.2f} seconds", print_time)
    ...
    t3 = time.time()
    logger_if(f"Post time taken: {t3 - t2:.2f} seconds", print_time)
    return image
